...
//    private static String type = "/image";
    private static String type = "/mall_image";
...
//    private static String modelPath = "/store";
    ...
    public static File createFile(String filePath) throws IOException {
        ...
        if (!file.isDirectory()) {
            //创建失败返回null
            if (!file.mkdirs()) {
                throw new MallException("文件目录创建失败...");
            }
        }
        // 判断这个文件是否存在，不存在就创建
        ...
        if (!file.exists()) {
            if (!file.createNewFile()) {
                throw new MallException("目标文件创建失败...");
            }
        }
        return file;
    }
    ...
    private static void isContains(String extName) {
        if (StringUtils.isNotEmpty(getExtStr())) {
            // 切割文件扩展名
            List<String> extensionList = MallUtil.stringToArrayStr(getExtStr());

            if (extensionList.size() > 0) {
                //判断
                if (!extensionList.contains(extName)) {
                    throw new MallException("上传文件的类型只能是：" + getExtStr());
                }
            } else {
                throw new MallException("上传文件的类型只能是：" + getExtStr());
            }
...
//    }
    ...
    public static String fileName(String extName){
        return MallUtil.getUuid() + RandomUtil.randomString(10) + "." + extName;
    }
    ...
    private static void checkSize(Long size) {
        ...
        if( fileSize > getSize()){
            throw new MallException("最大允许上传" + getSize() + " MB的文件, 当前文件大小为 " + fs + " MB");
        }
    }
    ...
    private static FileResultVo saveFile(MultipartFile multipartFile) throws IOException {
        if (null == multipartFile || multipartFile.isEmpty()) {
            throw new MallException("上传的文件对象不存在...");
        }
        // 文件名
