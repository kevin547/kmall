...
import com.jjsj.mall.system.dao.SystemStoreDao;
import com.jjsj.mall.system.model.SystemStore;
import com.jjsj.mall.system.request.SystemStoreRequest;
...
public class SystemStoreServiceImpl extends ServiceImpl<SystemStoreDao, SystemStore> implements SystemStoreService {
    ...
    public Boolean completeLyDelete(Integer id) {
        ...
        if (ObjectUtil.isNull(systemStore)) throw new MallException("提货点不存在!");
    ...
    private void splitLat(SystemStore systemStore) {
        if(!StringUtils.isBlank(systemStore.getLatitude())){
            List<String> list = MallUtil.stringToArrayStr(systemStore.getLatitude());
