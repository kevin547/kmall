...
import com.jjsj.constants.UserConstants;
...
import com.jjsj.mall.store.response.StoreOrderCountItemResponse;
...
import com.jjsj.mall.store.response.StoreOrderTopItemResponse;
...
import com.jjsj.mall.system.response.StoreOrderItemResponse;
 ...
 */
@Service
public class StoreOrderServiceImpl extends ServiceImpl<StoreOrderDao, StoreOrder> implements
    StoreOrderService {
    ...
    @Autowired
    private com.jjsj.mall.store.service.StoreOrderInfoService StoreOrderInfoService;
    ...
    */
    ...
    public List<String> addCartAgain(Integer userId, Integer productId, Integer cartNum, String orderUnique, String type, boolean isNew, Integer combinationId, Integer skillId, Integer bargainId) {
        ...
        if(null == existOrder) throw new MallException("订单不存在");
...
//        Long cacheKey = DateUtil.getTime()+userId;
        ...
        for (StoreOrderInfoVo so : orderAgainVo.getCartInfo()) {
            ...
            if(null == existProduct) throw new MallException("该商品已下架或者删除");
            ...
            if(existSPAttrValue.getStock() < cartNum) throw new MallException("该商品库存不足");
            // 添加有效商品至缓存 缓存为购物车对象
            if(isNew){
...
//        HashMap<Integer, SystemStoreStaff> systemStoreStaffList = systemStoreStaffService.getMapInId(clerkIdList);
    ...
    public boolean refund(StoreOrderRefundRequest request) {
        ...
        if(ObjectUtil.isNull(storeOrder)){throw new MallException("未查到订单");}
        if(!storeOrder.getPaid()){throw new MallException("未支付无法退款");}
...
//                storeNameAndCarNumString,storeOrder.getRefundReason(),"MALL",storeOrder.getRefundReasonWapExplain(),
//                "暂无"
//        );
//        wechatSendMessageForMinService.sendReFundEdMessage(forReFundEd, userService.getUserIdException());
    ...
    public boolean send(StoreOrderSendRequest request) {
        ...
        if (ObjectUtil.isNull(storeOrder)) throw new MallException("订单未能查到,不能发货!");
...
//        }
    ...
    public LogisticsResultVo getLogisticsInfo(Integer id) {
        ...
        if(StringUtils.isBlank(info.getDeliveryId())){
            //没有查询到快递信息
            throw new MallException("没有查询到快递信息");
        }
    ...
    public boolean editPrice(StoreOrderEditPriceRequest request) {
        ...
        // 订单不存在
        if(null == existOrder) {
            throw new MallException(Constants.RESULT_ORDER_NOTFOUND.replace("${orderCode}", request.getOrderId()));
        }
        // 订单已支付
        if(existOrder.getPaid()) {
            throw new MallException(Constants.RESULT_ORDER_PAYED.replace("${orderCode}", request.getOrderId()));
        }
        // 修改价格和原来价格相同
    ...
    public boolean confirmPayed(String orderId) {
        ...
        if(null == existOrder) throw new MallException(Constants.RESULT_ORDER_NOTFOUND.replace("${orderCode}", orderId));
    ...
    public boolean payOrderOffLine(Integer orderId) {
        ...
        if(null == existOrder) throw new MallException(
                Constants.RESULT_ORDER_NOTFOUND_IN_ID.replace("${orderId}", orderId+""));
        if(existOrder.getPaid()) throw new MallException(Constants.RESULT_ORDER_PAYED.replace("${orderCode}",existOrder.getOrderId()));
...
///////////////////////////////////////////////////////////////////////////////////////////////////// 以下为自定义方法
    ...
    private void subtractBill(StoreOrderRefundRequest request, String category, String type, String foundsType) {
        try{
            ...
            throw new MallException("更新退款佣金/积分失败");
        }
    }
...
//        if (StrUtil.isBlank(request.getExpressName())) throw new MallException("请选择快递公司");
