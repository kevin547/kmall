package com.jjsj.mall.store.utilService;
...
import com.jjsj.mall.seckill.service.StoreSeckillMangerService;
...
import com.jjsj.mall.system.response.SystemGroupDataOrderStatusPicResponse;
...
import com.jjsj.mall.wechat.vo.WechatSendMessageForDistrbution;
import com.jjsj.mall.wechat.vo.WechatSendMessageForPackage;
import com.jjsj.mall.wechat.vo.WechatSendMessageForPaySuccess;
...
import com.jjsj.mall.front.vo.OrderAgainItemVo;
...
//            }
    ...
    public ComputeOrderResponse computedOrder(OrderCreateRequest request, ConfirmOrderResponse cor, String orderKey, User currentUser){
        // 检查订单是否存在
        if(checkOrderExist(orderKey, currentUser.getUid())) throw new MallException(orderKey + "订单已存在");
        ...
        if(CollUtil.isEmpty(existPayType)) throw new MallException("选择支付方式有误");

        // 检测订单是否重复提交
        if(checkOrderSubmitAgain(orderKey)) throw new MallException("请勿重复提交订单");
        ...
        if(couponId > 0){
            ...
            if(!couponUseResult) throw new MallException("使用优惠券失败");
...
//            if(request.getAddressId() <= 0) throw new MallException("请选择收货地址");
...
//        storeOrder.setOrderId(MallUtil.getOrderNo(Constants.PAY_TYPE_WE_CHAT));
//
//        // 后置选择收货地址信息
...
//            storeOrder.setVerifyCode(MallUtil.randomCount(1111111111,999999999)+"");
...
//            if(null == existSystemStore) throw new MallException("暂无门店无法选择门店自提");
//            storeOrder.setStoreId(existSystemStore.getId());
//        }
//        boolean createdResult = storeOrderService.create(storeOrder);
//        if(!createdResult) throw new MallException("订单生成失败");
//
//        // 积分抵扣
...
//        }
//        if(!disIntegle) throw new MallException("使用积分抵扣失败");
//
...
//                if(existAttrValues.size() != 1) throw new MallException("未找到扣减库存的商品");
...
//                        throw new MallException("找不到对应的拼团团队信息");
//                    }
//                }
//                // 拼团扣减库存
//                // 拼团商品本身库存扣减
    ...
    public StoreOrder createOrder_v131(OrderCreateRequest request, ConfirmOrderResponse cor, String orderKey, User user){
        ...
        if(request.getShippingType() == 1){ // 发货
            ...
            if (storeSelfMention.equals("false")) {
                throw new MallException("请先联系管理员开启门店自提");
            }
            SystemStore systemStore = new SystemStore();
            systemStore.setId(request.getStoreId());
            systemStore.setIsShow(true);
            ...
            verifyCode = MallUtil.randomCount(1111111111,999999999)+"";
        ...
        if(request.getUseIntegral() && user.getIntegral() >= 0 && cor.getPriceGroup().getUsedIntegral() >= 0) {
            if (user.getIntegral() < cor.getPriceGroup().getUsedIntegral()) {
                throw new MallException("用户的积分不够抵扣，请刷新页面重新计算价格");
            }
        }
        ...
        if (request.getBargainId() > 0) {
            ...
            if (ObjectUtil.isNull(storeBargainUser)) throw new MallException("砍价用户信息不存在");
            if (!storeBargainUser.getStatus().equals(1)) {
                throw new MallException("活动已结束或订单已支付");
            }
        }

        // 库存扣减
        List<StoreCartResponse> cartInfoList = cor.getCartInfo();
        if (CollUtil.isEmpty(cartInfoList)) throw new MallException("购物详情信息不存在");
        ...
        String orderNo = MallUtil.getOrderNo("order");
        ...
        if (request.getPayType().equals(PayConstants.PAY_TYPE_WE_CHAT)) {
            ...
            // 目前只有余额支付
//            orderNo = MallUtil.getOrderNo(Constants.PAY_TYPE_YUE);
        }
        ...
        if (request.getSeckillId() > 0 || request.getBargainId() > 0 || request.getCombinationId() > 0) {
            ...
            if (request.getSeckillId() > 0) {
                ...
                if (storeSeckill.getStock().equals(0) || cartNum > storeSeckill.getStock()) {
                    throw new MallException("秒杀商品库存不足");
                }
                ...
                if (ObjectUtil.isNull(productAttrValue)) {
                    throw new MallException("秒杀主商品规格不存在");
                }
                if (productAttrValue.getStock() <= 0 || cartNum > productAttrValue.getStock()) {
                    throw new MallException("秒杀主商品规格库存不足");
                }
            ...
            if ( request.getBargainId() > 0) {
                ...
                if (storeBargain.getStock().equals(0) || cartNum > storeBargain.getStock()) {
                    throw new MallException("砍价商品库存不足");
                }
                ...
                if (ObjectUtil.isNull(bargainAttrValue)) {
                    throw new MallException("砍价商品规格不存在");
                }
                if (bargainAttrValue.getStock() <= 0 || bargainAttrValue.getQuota() <= 0 || cartNum > bargainAttrValue.getStock()) {
                    throw new MallException("砍价商品规格库存不足");
                }
                // 普通商品部分判断
                StoreProduct product = storeProductService.getById(productId);
                if (ObjectUtil.isNull(product) || product.getIsDel()) {
                    throw new MallException("砍价主商品不存在");
                }
                if (product.getStock().equals(0) || cartNum > product.getStock()) {
                    throw new MallException("砍价主商品库存不足");
                }
                // 主商品sku
                ...
                if (ObjectUtil.isNull(productAttrValue)) {
                    throw new MallException("砍价主商品规格不存在");
                }
                if (productAttrValue.getStock() <= 0 || cartNum > productAttrValue.getStock()) {
                    throw new MallException("砍价主商品规格库存不足");
                }
            ...
            if (request.getCombinationId() > 0) {
                ...
                if (storeCombination.getStock().equals(0) || cartNum > storeCombination.getStock()) {
                    throw new MallException("拼团商品库存不足");
                }
                ...
                if (ObjectUtil.isNull(combinationAttrValue)) {
                    throw new MallException("拼团商品规格不存在");
                }
                if (combinationAttrValue.getStock() <= 0 || combinationAttrValue.getQuota() <= 0 || cartNum > combinationAttrValue.getStock()) {
                    throw new MallException("拼团商品规格库存不足");
                }
                // 普通商品部分判断
                StoreProduct product = storeProductService.getById(productId);
                if (ObjectUtil.isNull(product) || product.getIsDel()) {
                    throw new MallException("拼团主商品不存在");
                }
                if (product.getStock().equals(0) || cartNum > product.getStock()) {
                    throw new MallException("拼团主商品库存不足");
                }
                // 主商品sku
            ...
            for (StoreCartResponse cartResponse : cartInfoList) {
                ...
                if (ObjectUtil.isNull(product) || product.getIsDel() || !product.getIsShow()) {
                    throw new MallException("购买的商品不存在或已下架");
                }
                if (product.getStock().equals(0) || cartNum > product.getStock()) {
                    throw new MallException("购买的商品库存不足");
                }
                ...
                if (ObjectUtil.isNull(productAttrValue)) {
                    throw new MallException("购买的商品规格不存在");
                }
                if (productAttrValue.getStock() <= 0 || cartNum > productAttrValue.getStock()) {
                    throw new MallException("购买的商品规格库存不足");
                }
...
//        };
        ...
        if(!Arrays.asList(deleteStatusList).contains(orderStatus)){
            throw new MallException("该订单无法删除");
        }
    }
...
//                queryWrapper.eq(StoreOrder::getShippingType, 1);
    ...
    public StoreSeckill validSecKill(StoreCart storeCartPram, User currentUser) {
        ...
        if(null == existSecKills) throw new MallException("该商品已下架或者删除");
        ...
        if(DateUtil.getTwoDateDays(DateUtil.nowDateTime(),stopDate) < 0){
            throw new MallException("秒杀商品已过期");
        }
        ...
        if(null == currentSecKillAttrValues || currentSecKillAttrValues.size() == 0){
            throw new MallException("未找到该商品信息");
        }
        ...
        if(unPayOrders.size() > 0) throw new MallException("您有秒杀待支付订单，请支付后再购买");

        // 判断是否达到上线
        if(null != userCurrentDaySecKillOrders && userCurrentDaySecKillOrders.size() >= existSecKill.getNum()){
            throw new MallException("您已经达到当前秒杀活动上限");
        }
    ...
    public MyRecord validBargain(StoreCart storeCartPram, User currentUser) {
        ...
        if (existBargain.getQuota() <= 0 || existBargain.getStock() <= 0) {// 销量等于限量
            throw new MallException("当前拼团商品已售罄");
        }

        // 判断砍价活动时间段
        long timeMillis = System.currentTimeMillis();
        if (timeMillis < existBargain.getStartTime()) {
            throw new MallException("砍价商品活动未开始");
        }
        if (timeMillis > existBargain.getStopTime()) {
            throw new MallException("砍价商品已过期");
        }
        ...
        if (surplusPrice.compareTo(BigDecimal.ZERO) > 0) {
            throw new MallException("请先完成砍价");
        }
        ...
        if(unPayOrders.size() > 0) throw new MallException("您有砍价待支付订单，请支付后再购买");
        ...
        if(CollUtil.isNotEmpty(userCurrentBargainOrders) && noRefundOrders.size() >= existBargain.getNum()){
            throw new MallException("您已经达到当前砍价活动上限");
        }
        MyRecord record = new MyRecord();
    ...
    public MyRecord validCombination(StoreCart storeCartPram, User currentUser) {
        ...
        if (timeMillis < existCombination.getStartTime()) {
            throw new MallException("拼团商品活动未开始");
        }
        if (timeMillis >= existCombination.getStopTime()) {
            throw new MallException("拼团商品已过期");
        }
        // 判断购买数量
        if (storeCartPram.getCartNum() > existCombination.getOnceNum()) {
            throw new MallException("购买数量超过单次拼团购买上限");
        }
        ...
        if (ObjectUtil.isNotNull(storeCartPram.getPinkId()) && storeCartPram.getPinkId() > 0) {
            ...
            if (countPeople >= existCombination.getPeople()) {
                throw new MallException("当前拼团人数已满");
            }
        }
        ...
        if (storeCartPram.getCartNum() > storeProductAttrValue.getQuota()) {
            throw new MallException("数量超过拼团商品库存上限");
        }
        ...
        if (CollUtil.isNotEmpty(userCombinationOrders)) {
            ...
            if(unPayOrders.size() > 0) throw new MallException("您有拼团待支付订单，请支付后再购买");
            ...
            if (CollUtil.isNotEmpty(noRefundOrders) && existCombination.getNum() <= payNum){
                throw new MallException("您已达到该商品拼团活动上限");
            }
            if ((payNum + storeCartPram.getCartNum()) > existCombination.getNum()) {
                throw new MallException("超过该商品拼团活动您的购买上限");
            }
        }
