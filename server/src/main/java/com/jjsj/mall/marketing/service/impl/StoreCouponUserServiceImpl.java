package com.jjsj.mall.marketing.service.impl;
...
import com.jjsj.mall.marketing.dao.StoreCouponUserDao;
...
//        lambdaQueryWrapper.orderByDesc(StoreCouponUser::getId);
    ...
    public Boolean receive(StoreCouponUserRequest request) {
        ...
        List<Integer> uidList = MallUtil.stringToArray(request.getUid());
        if(uidList.size() < 1){
            throw new MallException("请选择用户！");
        }
        ...
        if(storeCoupon.getIsLimited()){
            ...
            if(storeCoupon.getLastTotal() < uidList.size()){
                throw new MallException("当前剩余数量不够领取！");
            }
        }
        ...
        if(uidList.size() < 1){
            //都已经领取过了
            throw new MallException("当前用户已经领取过此优惠券了！");
    ...
    public boolean use(Integer id, List<Integer> productIdList, BigDecimal price) {
        ...
        if(storeCouponUser.getUseType() > 1){
            ...
            //设置优惠券所提供的集合
            List<Integer> primaryKeyIdList = MallUtil.stringToArray(storeCouponUser.getPrimaryKey());

            //取两个集合的交集，如果是false则证明没有相同的值
            if(storeCouponUser.getUseType() == 2 && !primaryKeyIdList.retainAll(productIdList)){
                throw new MallException("此优惠券为商品券，请购买相关商品之后再使用！");
            }

            if(storeCouponUser.getUseType() == 3 && !primaryKeyIdList.retainAll(categoryIdList)){
                throw new MallException("此优惠券为分类券，请购买相关分类下的商品之后再使用！");
            }
        }
    ...
    public boolean receiveAll(UserCouponReceiveRequest request, Integer userId, String type) {
        ...
        if(storeCouponList.size() < 1){
            ...
            throw new MallException("没有可以发放的优惠券！");
        }
    ...
    public boolean rollbackByCancelOrder(StoreOrder storeOrder) {
        try{
            ...
            throw new MallException("回滚优惠券信息失败");
        }
    }
    ...
    public void overdueTask() {
        ...
        if (!update) throw new MallException("批量更新优惠券过期动作失败");
    }
...
//                        .or(p -> p.eq(StoreCouponUser::getUseType , 2).apply(MallUtil.getFindInSetSql("primary_key", productIdStr)))
//                        .or(c -> c.eq(StoreCouponUser::getUseType , 3).apply(MallUtil.getFindInSetSql("primary_key", (ArrayList<Integer>) categoryIdList)))
                t -> t.eq(StoreCouponUser::getUseType, 1)
