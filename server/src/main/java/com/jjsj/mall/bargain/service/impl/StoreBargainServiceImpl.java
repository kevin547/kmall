...
import com.jjsj.constants.ProductConstants;
...
import com.jjsj.mall.bargain.dao.StoreBargainDao;
import com.jjsj.mall.bargain.model.StoreBargain;
import com.jjsj.mall.bargain.model.StoreBargainUser;
import com.jjsj.mall.bargain.request.StoreBargainRequest;
    ...
    */
    ...
    public boolean saveBargain(StoreBargainRequest request) {
        ...
        if (null == request.getAttrValue() || request.getAttrValue().size() < 1) {
            throw new MallException("请选择砍价商品的规格属性");
        }
        ...
        if (ObjectUtil.isNull(attrValueRequest.getQuota()) || attrValueRequest.getQuota() <= 0) {
            throw new MallException("活动限购数量必须大于0");
        }
        ...
        if (!save) throw new MallException("新增砍价商品失败");
        ...
        if(!saveAttrValue) throw new MallException("新增属性详情失败");
        ...
        if(!saveResult) throw new MallException("新增商品属性详情详情失败");
        ...
        if (!saveDesc) throw new MallException("新增商品富文本失败");
    ...
    public boolean updateBarhain(StoreBargainRequest request) {
        ...
        if (saveCount <= 0) {
            throw new MallException("编辑砍价商品失败");
        }
        ...
        singleAttrValue.setImage(systemAttachmentService.clearPrefix(singleAttrValue.getImage()));
        boolean saveAttrValue = attrValueService.save(singleAttrValue);
        if(!saveAttrValue) throw new MallException("编辑属性详情失败");
    ...
    public boolean updateBargainStatus(Integer id, boolean status) {
        ...
        if (ObjectUtil.isNull(temp) || temp.getIsDel()) {
            throw new MallException("砍价商品不存在");
        }
        if (status) {
            // 判断商品是否存在
    ...
    public StoreProductResponse getAdminDetail(Integer bargainId) {
        ...
        if (ObjectUtil.isNull(storeBargain)) throw new MallException("未找到对应砍价商品信息");
    ...
    public Boolean start(BargainFrontRequest request) {
        ...
        if (ObjectUtil.isNull(storeBargain)) throw new MallException("对应的砍价商品不存在");
        if (!storeBargain.getStatus())  throw new MallException("砍价活动已结束");
        User user = userService.getInfo();
        ...
        if (CollUtil.isNotEmpty(historyList)) {
            ...
            if (CollUtil.isNotEmpty(collect)) throw new MallException("请先完成当前砍价活动");
            // 判断是否达到参与砍价活动上限
            if (historyList.size() >= storeBargain.getNum()) {
                throw new MallException("您已达到当前砍价活动上限");
            }
        }
    ...
    public Boolean decProductStock(Integer bargainId, Integer num, Integer attrValueId, Integer productId, Integer uid) {
        ...
        if (CollUtil.isEmpty(attrvalues)) throw new MallException("未找到相关商品属性");
    ...
    public void stopAfterChange() {
        ...
        if (!b) {
            ...
            throw new MallException("砍价活动结束后更新用户状态失败");
        }
    }
    ...
    public StoreBargain getByIdException(Integer id) {
        ...
        if (ObjectUtil.isNull(storeBargain)) throw new MallException("砍价商品不存在或未开启");
        return storeBargain;
    }
