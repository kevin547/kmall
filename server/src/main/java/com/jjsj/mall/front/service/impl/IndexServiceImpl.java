...
import com.jjsj.mall.front.response.IndexInfoItemResponse;
...
import com.jjsj.mall.front.service.IndexService;
...
public class IndexServiceImpl implements IndexService {
     ...
     */
    @Override
    public IndexProductBannerResponse getProductBanner(int type,
        ...
        indexProductBannerResponse
            .setList(productService.getIndexProduct(request, pageParamRequest).getList());
    ...
    public IndexInfoResponse getIndexInfo() {
        ...
        indexInfoResponse.setBanner(systemGroupDataService
            .getListMapByGid(Constants.GROUP_DATA_ID_INDEX_BANNER)); //首页banner滚动图
        indexInfoResponse.setMenus(
            systemGroupDataService.getListMapByGid(Constants.GROUP_DATA_ID_INDEX_MENU)); //导航模块
        indexInfoResponse.setRoll(systemGroupDataService
            .getListMapByGid(Constants.GROUP_DATA_ID_INDEX_NEWS_BANNER)); //首页滚动新闻
        indexInfoResponse.setActivity(systemGroupDataService
            .getListMapByGid(Constants.GROUP_DATA_ID_INDEX_ACTIVITY_BANNER)); //首页活动区域图片
        ...
        IndexInfoItemResponse indexInfoItemResponse = MallUtil
            .mapStringToObj(info, IndexInfoItemResponse.class);
        ...
        indexInfoItemResponse.setBastList(
            productService.getIndexProduct(request, pageParamRequest).getList()); //精品推荐个数
        ...
        indexInfoItemResponse.setFirstList(
            productService.getIndexProduct(request, pageParamRequest).getList()); //首发新品个数
        ...
        indexInfoItemResponse.setBastBanner(systemGroupDataService
            .getListMapByGid(Constants.GROUP_DATA_ID_INDEX_BEST_BANNER)); //首页精品推荐图片
        ...
        if (productService.getIndexProduct(request, pageParamRequest) != null) {
            indexInfoResponse.setBenefit(
                productService.getIndexProduct(request, pageParamRequest).getList()); //首页促销单品
        }

        request.setIsBenefit(false);
        request.setIsHot(true);
        pageParamRequest.setLimit(limit);
        indexInfoResponse.setLikeInfo(
            productService.getIndexProduct(request, pageParamRequest).getList()); //热门榜单

        indexInfoResponse.setLovely(null);//首发新品广告图
        List<HashMap<String, Object>> lovelyList = systemGroupDataService
            .getListMapByGid(Constants.GROUP_DATA_ID_INDEX_NEW_BANNER);
        if (lovelyList != null && lovelyList.size() > 0) {
            indexInfoResponse.setLovely(
                systemGroupDataService.getListMapByGid(Constants.GROUP_DATA_ID_INDEX_NEW_BANNER)
                    .get(0));//首发新品广告图
        }
        indexInfoResponse.setExplosiveMoney(systemGroupDataService
            .getListMapByGid(Constants.GROUP_DATA_ID_INDEX_EX_BANNER));//首页超值爆款
