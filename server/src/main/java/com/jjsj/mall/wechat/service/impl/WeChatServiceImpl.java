...
import com.jjsj.mall.wechat.vo.MediaCountVo;
...
public class WeChatServiceImpl implements WeChatService {
    ...
    private void getToken(){
        ...
        if(!data.containsKey("access_token")){
            throw new MallException("微信token获取失败：" + data.getString("errmsg"));
        }
    ...
    private void getAppInfo() {
        ...
        if(StringUtils.isBlank(this.appId)){
            throw new MallException("微信appId未设置");
        }

        if(StringUtils.isBlank(this.secret)){
            throw new MallException("微信secret未设置");
        }
    }
    ...
    private void getProgramAppInfo() {
        ...
        if(StringUtils.isBlank(this.programAppId)){
            throw new MallException("微信小程序appId未设置");
        }

        if(StringUtils.isBlank(this.programAppSecret)){
            throw new MallException("微信小程序secret未设置");
        }
    }
    ...
    private String postReturnBuffer(String url, HashMap<String, Object> data){
        try{
            ...
            if(StringUtils.contains(response,"errcode")){
                ...
                throw new MallException("微信生成二维码异常");
            }
            return MallUtil.getBase64Image(Base64.encodeBase64String(bytes));
    ...
    private JSONObject checkResult(JSONObject result){
        ...
        if(result.equals("")){
            throw new MallException("微信平台接口异常，没任何数据返回！");
        }
        ...
        if(result.containsKey("errmsg")){
            throw new MallException("微信接口调用失败：" + result.getString("errcode") + result.getString("errmsg"));
        }
    ...
    public JSONObject memberBatchTags(String id, String data) {
        ...
        HashMap<String, Object> map = new HashMap<>();
        map.put("openid_list", MallUtil.stringToArrayStr(data));
        map.put("tagid", id);
        return post(this.url, map);
    }
    ...
    public UserRechargePaymentResponse response(CreateOrderResponseVo responseVo) {
        ...
        //重新计算签名
        response.setPaySign(MallUtil.getSign(map, systemConfigService.getValueByKey(Constants.CONFIG_KEY_PAY_WE_CHAT_APP_KEY)));
    ...
    public List<ProgramCategoryVo> getProgramCategory() {
        ...
        if(null == data || data.size() < 1){
            throw new MallException("请先设置小程序行业信息");
        }

        return MallUtil.jsonToListClass(data.toJSONString(), ProgramCategoryVo.class);
    }
    ...
    public List<ProgramTempVo> getProgramPublicTempList(int page) {
        ...
        return MallUtil.jsonToListClass(data.toJSONString(), ProgramTempVo.class);
    }
    ...
    public List<ProgramTempKeywordsVo> getWeChatKeywordsByTid(Integer tid) {
        ...
        JSONArray data = jsonObject.getJSONArray("data");
        if(null == data || data.size() < 1){
            throw new MallException("微信没有返回关键词数据");
        }

        return MallUtil.jsonToListClass(data.toJSONString(), ProgramTempKeywordsVo.class);
    }
    ...
    public String programAddMyTemp(ProgramAddMyTempVo programAddMyTempVo) {
        ...
        if(StringUtils.isBlank(data.getString("priTmplId"))){
            throw new MallException("微信没有返回模板id");
        }
    ...
    public Object getJsSdkConfig(String url) {
        try {
            ...
            throw new MallException("url无法解析！");
        }
        setJsApiTicket();
        setJsSDKSign(url);
    ...
    private void setJsSDKSign(String url) {
        ...
        try {
            ...
            try {
                ...
                throw new MallException("获取JS SDK配置失败" + e.getMessage());
            }
            this.signature = MallUtil.byteToHex(crypt.digest());
    ...
    private List<String> getJsApiList() {
        return MallUtil.stringToArrayStr(WeChatConstants.PUBLIC_API_JS_API_SDK_LIST);
    }
