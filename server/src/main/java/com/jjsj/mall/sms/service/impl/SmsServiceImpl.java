...
import com.jjsj.mall.sms.request.SendSmsVo;
import com.jjsj.mall.sms.request.SmsApplyTempRequest;
import com.jjsj.mall.sms.request.SmsModifySignRequest;
...
public class SmsServiceImpl implements SmsService {
    ...
    /**
     * 添加到短信队列 验证码特殊处理其他的参数自行根据要求处理 参数处理逻辑 {code:value,code1:value1}
     *
    ...
    private Boolean pushCodeToList(String phone, Integer tag, HashMap<String, Object> pram) {
        ...
        switch (tag) {
            case SmsConstants.SMS_CONFIG_TYPE_VERIFICATION_CODE: // 验证码 特殊处理 code
                //获取短信验证码过期时间
                String codeExpireStr = systemConfigService
                    .getValueByKey(Constants.CONFIG_KEY_SMS_CODE_EXPIRE);
                if (StringUtils.isBlank(codeExpireStr) || Integer.parseInt(codeExpireStr) == 0) {
                    codeExpireStr = Constants.NUM_FIVE + "";// 默认5分钟过期
                }
                Integer code = MallUtil.randomCount(111111, 999999);
                ...
                // 将验证码存入redis
                redisUtil.set(userService.getValidateCodeRedisKey(phone), code,
                    Long.valueOf(codeExpireStr), TimeUnit.MINUTES);
                break;
    ...
    public Boolean sendCode(SendSmsVo sendSmsVo) {
        ...
        try {
            ...
            Map<String, Object> map = (Map<String, Object>) JSONObject
                .parseObject(sendSmsVo.getParam());
            MultiValueMap<String, Object> param = new LinkedMultiValueMap<>();
            param.add("phone", sendSmsVo.getMobile());
            param.add("temp_id", sendSmsVo.getTemplate());
            map.entrySet().stream().forEach(entry -> param
                .add(StrUtil.format(SmsConstants.SMS_COMMON_PARAM_FORMAT, entry.getKey()),
                    entry.getValue()));
            System.out.println("============发送短信=========header = " + header);
            result = restTemplateUtil.postFromUrlencoded(
                OnePassConstants.ONE_PASS_API_URL + OnePassConstants.ONE_PASS_API_SEND_URI, param,
                header);
        } catch (Exception e) {
            //接口请求异常，需要重新发送
            e.printStackTrace();
        ...
        if (resultCode == Constants.HTTPSTATUS_CODE_SUCCESS) {
            try {
                ...
                SmsRecord smsRecord = new SmsRecord(sendSmsVo.getUid(), sendSmsVo.getMobile(),
                    sendSmsVo.getContent(),
...
//                    SendSmsVo smsVo = JSONObject.toJavaObject(JSONObject.parseObject(data.toString()), SendSmsVo.class);
     ...
     */
    @Override
    public void push(String phone, String tempKey, Integer msgTempId,
        HashMap<String, Object> mapPram) {
    ...
    public Boolean modifySign(SmsModifySignRequest request) {
        ...
        onePassUtil
            .postFrom(OnePassConstants.ONE_PASS_API_URL + OnePassConstants.ONE_PASS_SMS_MODIFY_URI,
                map, header);
        return Boolean.TRUE;
    }
    ...
    public MyRecord temps(PageParamRequest pageParamRequest) {
        ...
        JSONObject post = postFrom(
            OnePassConstants.ONE_PASS_API_URL + OnePassConstants.ONE_PASS_TEMP_LIST_URI, param,
            header);
    ...
    public Boolean applyTempMessage(SmsApplyTempRequest request) {
        ...
        onePassUtil
            .postFrom(OnePassConstants.ONE_PASS_API_URL + OnePassConstants.ONE_PASS_TEMP_APPLY_URI,
                param, header);
        return Boolean.TRUE;
    }
    ...
    public MyRecord applys(Integer type, PageParamRequest pageParamRequest) {
        ...
        JSONObject post = onePassUtil
            .postFrom(OnePassConstants.ONE_PASS_API_URL + OnePassConstants.ONE_PASS_APPLYS_LIST_URI,
                param, header);
        JSONObject jsonObject = post.getJSONObject("data");
        JSONArray jsonArray = jsonObject.getJSONArray("data");
        MyRecord myRecord = new MyRecord();
    ...
    public Boolean sendOrderRefundApplyNotice(String phone, String orderNo, String realName) {
        ...
        map.put("admin_name", realName);
        map.put("order_id", orderNo);
        return pushCodeToList(phone, SmsConstants.SMS_CONFIG_TYPE_ADMIN_CONFIRM_TAKE_OVER_SWITCH,
            map);
    }
     ...
     */
    @Override
    public Boolean sendOrderDeliverNotice(String phone, String nickName, String storeName,
        String orderNo) {
        HashMap<String, Object> map = CollUtil.newHashMap();
     ...
     * post请求from表单模式提交
     */
    private JSONObject postFrom(String url, MultiValueMap<String, Object> param,
    ...
    private JSONObject checkResult(String result) {
        if (StringUtils.isBlank(result)) {
            throw new MallException("短信平台接口异常，没任何数据返回！");
        }
        ...
        try {
            ...
            throw new MallException("短信平台接口异常！");
        }
        if (SmsConstants.SMS_ERROR_CODE.equals(jsonObject.getInteger("status"))) {
            throw new MallException("短信平台接口" + jsonObject.getString("msg"));
        }
        return jsonObject;
    }
}
