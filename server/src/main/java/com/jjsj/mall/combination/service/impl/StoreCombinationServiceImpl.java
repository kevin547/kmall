package com.jjsj.mall.combination.service.impl;
...
import com.jjsj.mall.combination.response.StoreCombinationResponse;
...
import com.jjsj.mall.combination.dao.StoreCombinationDao;
import com.jjsj.mall.combination.model.StoreCombination;
import com.jjsj.mall.combination.model.StorePink;
import com.jjsj.mall.combination.request.StoreCombinationRequest;
    ...
    */
    ...
    public Boolean saveCombination(StoreCombinationRequest request) {
        ...
        if (stopTime <= System.currentTimeMillis()) {
            throw new MallException("活动结束时间不能小于当前时间");
        }
        ...
        if (!save) throw new MallException("新增拼团商品失败");
        ...
        if(request.getSpecType()) { // 多规格
            if (CollUtil.isNotEmpty(request.getAttr()) && request.getAttr().size() > 0) {
                ...
                if (!attrSave) throw new MallException("新增拼团商品属性名失败");
            }
        } else { //单规格
        ...
        if (null != request.getAttrValue() && request.getAttrValue().size() > 0) {
            ...
            if(storeProductAttrValues.size() > 0){
                ...
                if(!saveOrUpdateResult) throw new MallException("新增拼团商品属性详情失败");
            }
        }
        // 处理富文本
        ...
        if (!descSave) throw new MallException("新增拼团商品详情失败");
        return save;
    }
    ...
    public Boolean deleteById(Integer id) {
        ...
        if (combination.getIsShow().equals(true) && combination.getStartTime() <= timeMillis && timeMillis <= combination.getStopTime()) {
            throw new MallException("活动开启中，商品不支持删除");
        }
    ...
    public Boolean updateCombination(StoreCombinationRequest request) {
        ...
        if (ObjectUtil.isNull(existCombination) || existCombination.getIsDel()) throw new MallException("拼团商品不存在");
        ...
        if (existCombination.getIsShow().equals(true) && existCombination.getStartTime() <= timeMillis && timeMillis <= existCombination.getStopTime()) {
            throw new MallException("活动开启中，商品不支持修改");
        }
        ...
        if (!update) throw new MallException("编辑拼团商品失败");
        ...
        if (request.getSpecType()) { // 单规格不处理规格属性
            ...
            if (!updateAttr) throw new MallException("编辑拼团商品属性失败");
        }
        ...
        if(null != request.getAttrValue() && request.getAttrValue().size() > 0){
            ...
            if(!resultSave) throw new MallException("编辑属性详情结果失败");
        }
        // 处理富文本
        if (StrUtil.isNotBlank(request.getContent())) {
            ...
            if (!saveDesc) throw new MallException("编辑拼团商品详情失败");
        }
        return update;
    }
    ...
    public StoreProductResponse getAdminDetail(Integer id) {
        ...
        if (ObjectUtil.isNull(storeCombination)) throw new MallException("未找到对应商品信息");
    ...
    public CombinationDetailResponse getH5Detail(Integer comId) {
        ...
        if (ObjectUtil.isNull(storeCombination) || storeCombination.getIsDel()) {
            throw new MallException("对应拼团商品不存在");
        }
...
//        List<HashMap<String, Object>> attrValuesCombination = genratorSkuInfo(storeCombination.getId(), storeCombination, valueList, Constants.PRODUCT_TYPE_PINGTUAN);
    ...
    public GoPinkResponse goPink(Integer pinkId) {
        ...
        if (ObjectUtil.isNull(teamPink) || teamPink.getIsRefund()) throw new MallException("对应的拼团不存在");
        StoreCombination storeCombination = getById(teamPink.getCid());
        if (ObjectUtil.isNull(storeCombination) || storeCombination.getIsDel()) throw new MallException("对应拼团商品不存在");
        User user = userService.getInfo();
    ...
    public Boolean removePink(StorePinkRequest storePinkRequest) {
        ...
        if (ObjectUtil.isNull(userPink)) throw new MallException("未查到拼团信息，无法取消");
        if (userPink.getIsRefund()) throw new MallException("拼团订单已退款");
        // 获取是否拼团成功
        if (userPink.getStatus() == 2) {
            throw new MallException("拼团已完成，无法取消");
        }
        ...
        if (userPink.getStatus() == 3) {
            throw new MallException("拼团已申请取消");
        }

        StoreOrder order = storeOrderService.getByOderId(userPink.getOrderId());
        if (ObjectUtil.isNull(order) || order.getIsDel()) throw new MallException("拼团订单不存在");
        if (order.getStatus() == -1 && order.getRefundStatus() != 0) {
            throw new MallException("拼团订单已进入退款流程");
        }
        ...
        if (!apply) throw new MallException("订单申请退款失败");
    ...
    public Boolean decProductStock(StoreOrder storeOrder, Integer num, Integer attrValueId, Integer productId, User user) {
        ...
        if(CollUtil.isEmpty(attrvalues)) throw new MallException("未找到相关商品属性信息");
        ...
        List<StoreProductAttrValue> existAttrValues = currentProAttrValues.stream().filter(e ->
                ...
                .collect(Collectors.toList());
        if (CollUtil.isEmpty(existAttrValues)) throw new MallException("未找到扣减库存的商品");
    ...
    public StoreCombination getByIdException(Integer combinationId) {
        ...
        if (ObjectUtil.isNull(storeCombination)) throw new MallException("拼团商品不存在或未开启");
        return storeCombination;
    }
    ...
    private void clearNotCheckedAndValidationPrice(StoreCombinationRequest request) {
        ...
        if (CollUtil.isEmpty(request.getAttrValue())) {
            throw new MallException("请选择 规格");
        }
        for (StoreProductAttrValueRequest attr : request.getAttrValue()) {
            if (ObjectUtil.isNull(attr.getPrice()) || attr.getPrice().compareTo(BigDecimal.ZERO) <= 0){
                throw new MallException("请正确输入 拼团价格");
            }
            if (ObjectUtil.isNull(attr.getQuota()) || attr.getQuota() <= 0) {
                throw new MallException("请正确输入 限量");
            }
            if (attr.getQuota() > attr.getStock()) {
                throw new MallException("限量 不能大于库存");
            }
