...
import com.jjsj.mall.user.dao.UserBrokerageRecordDao;
 ...
 */
@Service
public class UserBrokerageRecordServiceImpl extends
    ServiceImpl<UserBrokerageRecordDao, UserBrokerageRecord> implements UserBrokerageRecordService {

    private static final Logger logger = LoggerFactory
        .getLogger(UserBrokerageRecordServiceImpl.class);
    ...
    public void brokerageThaw() {
        ...
        for (UserBrokerageRecord record : thawList) {
            ...
            Boolean execute = transactionTemplate.execute(e -> {
                updateById(record);
                userService.operationBrokerage(record.getUid(), record.getPrice(),
                    user.getBrokeragePrice(), "add");
                return Boolean.TRUE;
            });
            if (!execute) {
    ...
    public BigDecimal getYesterdayIncomes(Integer uid) {
        ...
        lqw.between(UserBrokerageRecord::getUpdateTime, dateLimit.getStartTime(),
            dateLimit.getEndTime());
        List<UserBrokerageRecord> recordList = dao.selectList(lqw);
        if (CollUtil.isEmpty(recordList)) {
            return BigDecimal.ZERO;
        }
        return recordList.stream().map(UserBrokerageRecord::getPrice)
     ...
     */
    @Override
    public PageInfo<SpreadCommissionDetailResponse> findDetailListByUid(Integer uid,
        PageParamRequest pageParamRequest) {
        Page<UserBrokerageRecord> recordPage = PageHelper
        ...
        for (UserBrokerageRecord record : list) {
            ...
            responseList
                .add(new SpreadCommissionDetailResponse(month, getListByUidAndMonth(uid, month)));
        }
    ...
    public Integer getSpreadCountByUid(Integer uid) {
        ...
        lqw.eq(UserBrokerageRecord::getUid, uid);
        lqw.eq(UserBrokerageRecord::getLinkType,
            BrokerageRecordConstants.BROKERAGE_RECORD_LINK_TYPE_ORDER);
        lqw.eq(UserBrokerageRecord::getStatus,
     ...
     */
    @Override
    public List<UserBrokerageRecord> findSpreadListByUid(Integer uid,
        PageParamRequest pageParamRequest) {
    ...
    public Map<String, Integer> getSpreadCountByUidAndMonth(Integer uid, List<String> monthList) {
        ...
        queryWrapper
            .apply(StrUtil.format("left(update_time, 7) in ({})", StrUtil.join(",", monthList)));
        queryWrapper.groupBy("left(update_time, 7)");
        List<UserBrokerageRecord> list = dao.selectList(queryWrapper);
        ...
        list.forEach(record -> {
            map.put(DateUtil.dateToStr(record.getUpdateTime(), Constants.DATE_FORMAT_MONTH),
                record.getUid());
        });
        return map;
    }
    ...
    public BigDecimal getTotalSpreadPriceBydateLimit(String dateLimit) {
        ...
        if (StrUtil.isNotBlank(dateLimit)) {
            ...
            lqw.between(UserBrokerageRecord::getUpdateTime, dateLimitVo.getStartTime(),
                dateLimitVo.getEndTime());
        }
        List<UserBrokerageRecord> list = dao.selectList(lqw);
        if (CollUtil.isEmpty(list)) {
            return BigDecimal.ZERO;
        }
        return list.stream().map(UserBrokerageRecord::getPrice)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    }
     ...
     */
    @Override
    public PageInfo<UserBrokerageRecord> getFundsMonitorDetail(Integer uid, String dateLimit,
        PageParamRequest pageParamRequest) {
        Page<UserBrokerageRecord> page = PageHelper
            .startPage(pageParamRequest.getPage(), pageParamRequest.getLimit());
        LambdaQueryWrapper<UserBrokerageRecord> lqw = new LambdaQueryWrapper<>();
        lqw.eq(UserBrokerageRecord::getUid, uid);
//        lqw.eq(UserBrokerageRecord::getLinkType, BrokerageRecordConstants.BROKERAGE_RECORD_LINK_TYPE_ORDER);
        ...
        if (StrUtil.isNotBlank(dateLimit)) {
            ...
            lqw.between(UserBrokerageRecord::getUpdateTime, utilVo.getStartTime(),
                utilVo.getEndTime());
        }
        lqw.orderByDesc(UserBrokerageRecord::getUpdateTime);
        List<UserBrokerageRecord> recordList = dao.selectList(lqw);
