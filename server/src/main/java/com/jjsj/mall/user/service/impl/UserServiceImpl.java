...
import com.jjsj.mall.authorization.manager.TokenManager;
...
import com.jjsj.mall.front.request.RegisterRequest;
import com.jjsj.mall.front.request.UserBindingRequest;
import com.jjsj.mall.front.response.LoginResponse;
import com.jjsj.mall.front.response.UserCenterOrderStatusNumResponse;
...
public class UserServiceImpl extends ServiceImpl<UserDao, User> implements UserService {
    ...
    public PageInfo<UserResponse> getList(UserSearchRequest request,
        ...
        if (!StringUtils.isBlank(request.getGroupId())) {
            lambdaQueryWrapper.apply(MallUtil.getFindInSetSql("group_id", request.getGroupId()));
        }

        if (!StringUtils.isBlank(request.getLabelId())) {
            lambdaQueryWrapper.apply(MallUtil.getFindInSetSql("tag_id", request.getLabelId()));
        }

        if (!StringUtils.isBlank(request.getLevel())) {
            lambdaQueryWrapper.in(User::getLevel, MallUtil.stringToArray(request.getLevel()));
        }
        ...
        if (StringUtils.isNotBlank(request.getCountry())) {
            ...
            if (StrUtil.isNotBlank(request.getCity())) {
                ...
                lambdaQueryWrapper
                    .likeLeft(User::getAddres, request.getProvince() + "," + request.getCity());
            }
        }
...
//                    lambdaQueryWrapper.between(User::getUpdateTime, dateLimit.getStartTime(), dateLimit.getEndTime());
                    lambdaQueryWrapper.between(User::getCreateTime, dateLimit.getStartTime(),
...
//                    lambdaQueryWrapper.notBetween(User::getUpdateTime, dateLimit.getStartTime(), dateLimit.getEndTime());
                    lambdaQueryWrapper.between(User::getLastLoginTime, dateLimit.getStartTime(),
                        dateLimit.getEndTime());
                    break;
                case 3://未访问
//                    lambdaQueryWrapper.apply(" and create_time = last_login_time");
                    lambdaQueryWrapper.notBetween(User::getLastLoginTime, dateLimit.getStartTime(),
                        dateLimit.getEndTime());
                    break;
                default://全部
                    lambdaQueryWrapper.between(User::getCreateTime, dateLimit.getStartTime(),
        ...
        for (User user : userList) {
            ...
            if (null == user.getSpreadUid() || user.getSpreadUid() == 0) {
                ...
                userResponse
                    .setSpreadNickname(userDao.selectById(user.getSpreadUid()).getNickname());
            }
    ...
    public boolean updateFounds(UserOperateFundsRequest request, boolean isSaveBill) {
        UserOperateFundsVo userOperateFundsVo = new UserOperateFundsVo(request.getUid(),
            request.getFoundsCategory(), getFounds(request));
        //增加日志， 创建计算数值之前的数据
        if (isSaveBill) {
    ...
    public boolean updateIntegralMoney(UserOperateIntegralMoneyRequest request) {
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            if (request.getMoneyValue().compareTo(BigDecimal.ZERO) > 0) {
                ...
                if (request.getMoneyType() == 1) {// 增加
                    ...
                    operationNowMoney(user.getUid(), request.getMoneyValue(), user.getNowMoney(),
                        "add");
                } else {
                    userBill.setPm(0);
            ...
            if (request.getIntegralValue() > 0) {
                ...
                if (request.getIntegralType() == 1) {// 增加
                    ...
                    userBill.setType(Constants.USER_BILL_TYPE_SYSTEM_ADD);
                    userBill.setBalance(
                        new BigDecimal(user.getIntegral() + request.getIntegralValue()));
                    userBill.setMark(StrUtil.format("后台操作增加了{}积分", request.getIntegralValue()));

                    userBillService.save(userBill);
                    operationIntegral(user.getUid(), request.getIntegralValue(), user.getIntegral(),
                        "add");
                } else {
                    userBill.setPm(0);
                    userBill.setType(Constants.USER_BILL_TYPE_SYSTEM_SUB);
        ...
        if (!execute) {
            throw new MallException("修改积分/余额失败");
        }
        return execute;
    }
...
//        lambdaUpdateWrapper.setEntity(user);
        ...
        if (null != user.getBrokeragePrice()
    ...
    public boolean group(String id, String groupIdValue) {
        if (StrUtil.isBlank(id)) {
            throw new MallException("会员编号不能为空");
        }
        if (StrUtil.isBlank(groupIdValue)) {
            throw new MallException("分组id不能为空");
        }

        //循环id处理
        List<Integer> idList = MallUtil.stringToArray(id);
        idList = idList.stream().distinct().collect(Collectors.toList());
        List<User> list = getListInUid(idList);
        if (CollUtil.isEmpty(list)) {
            throw new MallException("没有找到用户信息");
        }
    ...
    public LoginResponse register(RegisterRequest request, String ip) throws Exception {
        ...
        if (user == null) {
            //此用户不存在，去帮他注册
            request.setPassword("Abc" + MallUtil.randomCount(10000, 99999));
    ...
    public LoginResponse login(LoginRequest request) throws Exception {
        ...
        String password = MallUtil.encryptPassword(request.getPassword(), request.getPhone());
    ...
    public boolean password(PasswordRequest request) {
        ...
        //密码
        user.setPwd(MallUtil.encryptPassword(request.getPassword(), user.getAccount()));
    ...
    public User getUserPromoter() {
        ...
        //获取分销
        //检测商城是否开启分销功能
        String isOpen = systemConfigService
            .getValueByKey(Constants.CONFIG_KEY_STORE_BROKERAGE_IS_OPEN);
        if (StringUtils.isBlank(isOpen) || isOpen.equals("0")) {
            user.setIsPromoter(false);
        } else {
            String type = systemConfigService
    ...
    public User getInfoException() {
        User user = getInfo();
        if (user == null) {
            throw new MallException("用户信息不存在！");
        }
...
//        }
    ...
    public Map<Object, Object> getAddUserCountGroupDate(String date) {
        ...
        for (User user : list) {
            map.put(DateUtil.dateToStr(user.getCreateTime(), Constants.DATE_FORMAT_DATE),
                user.getUid());
        }
        return map;
    }
    ...
    public boolean bind(UserBindingRequest request) {
        ...
        User user = getUserByAccount(request.getPhone());
        if (null != user) {
            throw new MallException("此手机号码已被注册");
        }
    ...
    public UserCenterResponse getUserCenter() {
        ...
        List<StoreCouponUserResponse> storeCoupons = storeCouponUserService
            .getListFront(getUserIdException(), pageParamRequest);
        ...
        if (memberFuncStatus == 0) {
            ...
            if (null != userLevel) {
                SystemUserLevel systemUserLevel = systemUserLevelService
                    .getByLevelId(userLevel.getLevelId());
    ...
    public UserCommissionResponse getCommission() {
        ...
        //昨天的佣金
        userCommissionResponse.setLastDayCount(userBillService
            .getSumBigDecimal(0, getUserId(), Constants.USER_BILL_CATEGORY_MONEY,
                Constants.SEARCH_DATE_YESTERDAY, null));
        userCommissionResponse.setExtractCount(userBillService
            .getSumBigDecimal(0, getUserId(), Constants.USER_BILL_CATEGORY_MONEY, null,
    ...
    public boolean tag(String id, String tagIdValue) {
        ...
        if (StrUtil.isBlank(tagIdValue)) {
            throw new MallException("标签id不能为空");
        }
    ...
    public List<UserSpreadPeopleItemResponse> getSpreadPeopleList(
        ...
        map.put("userIdList",
            userIdList.stream().map(String::valueOf).distinct().collect(Collectors.joining(",")));
    ...
    public String token(User user) throws Exception {
        TokenModel token = tokenManager.createToken(user.getAccount(), user.getUid().toString(),
            Constants.USER_TOKEN_REDIS_KEY_PREFIX);
    ...
    private User saveUser(RegisterRequest request, String ip) {
        ...
        user.setPwd(MallUtil.encryptPassword(request.getPassword(), request.getPhone()));
    ...
    public User registerPhone(String phone, Integer spreadUid, String clientIp) {
        ...
        String password = "Abc" + MallUtil.randomCount(10000, 99999);
        user.setPwd(MallUtil.encryptPassword(password, phone));
        user.setPhone(phone);
        user.setUserType(Constants.USER_LOGIN_TYPE_H5);
...
//                storeCouponUser.setUid(userId);
        ...
        if (!execute) {
            throw new MallException("创建用户失败!");
        }
        // 赠送客户优惠券
     ...
     */
    @Override
    public Boolean operationBrokerage(Integer uid, BigDecimal price, BigDecimal brokeragePrice,
        String type) {
        UpdateWrapper<User> updateWrapper = new UpdateWrapper<>();
        if (type.equals("add")) {
     ...
     */
    @Override
    public Boolean operationNowMoney(Integer uid, BigDecimal price, BigDecimal nowMoney,
        String type) {
        UpdateWrapper<User> updateWrapper = new UpdateWrapper<>();
        if (type.equals("add")) {
     ...
     */
    @Override
    public Boolean operationIntegral(Integer uid, Integer integral, Integer nowIntegral,
        String type) {
        UpdateWrapper<User> updateWrapper = new UpdateWrapper<>();
        if (type.equals("add")) {
     ...
     */
    @Override
    public PageInfo<User> getAdminSpreadPeopleList(String storeBrokerageStatus, String keywords,
        String dateLimit, PageParamRequest pageRequest) {
        ...
        lqw.select(User::getUid, User::getNickname, User::getRealName, User::getPhone,
            User::getAvatar, User::getSpreadCount, User::getBrokeragePrice, User::getSpreadUid);
...
//        }
    ...
    public Boolean checkBingSpread(User user, Integer spreadUid, String type) {
        ...
        if (type.equals("old")) {
            // 判断分销关系绑定类型（所有、新用户）
            String bindType = systemConfigService
                .getValueByKey(Constants.CONFIG_KEY_STORE_BROKERAGE_BIND_TYPE);
    ...
    private String getTitle(UserOperateFundsRequest request) {
        ...
        return Constants.USER_BILL_OPERATE_LOG_TITLE.replace("{$title}", request.getTitle())
            .replace("{$operate}", operate).replace("{$founds}", founds);
    }
    ...
    private void checkFounds(UserOperateFundsRequest request) {
        ...
        if (value == null) {
            throw new MallException("请选择正确的操作类型");
        }

        int result = value.subtract(request.getValue()).compareTo(BigDecimal.ZERO);
        if (result < 0) {
            if (request.getFoundsCategory().equals("integral")) {
                throw new MallException("此用户当前积分为 " + value + "， 需要减少的积分不能大于 " + value);
            }
            throw new MallException("此用户当前资金为 " + value + "， 需要减少的资金不能大于 " + value);
        }
    }
     ...
     */
    @Override
    public Object getInfoByCondition(Integer userId, Integer type,
        PageParamRequest pageParamRequest) {
        switch (type) {
            case 0:
...
//                return storeOrderService.getList(sor, pageParamRequest);
            ...
            case 3:
                ...
                scur.setUid(userId);
                PageInfo<StoreCouponUserResponse> pageInfo = storeCouponUserService
                    .getList(scur, pageParamRequest);
    ...
    public TopDetail getTopDetail(Integer userId) {
        ...
        topDetail.setMothConsumeCount(
            storeOrderService.getSumBigDecimal(userId, Constants.SEARCH_DATE_MONTH));
        topDetail.setAllConsumeCount(storeOrderService.getSumBigDecimal(userId, null));
        topDetail.setMothOrderCount(
            storeOrderService.getOrderCount(userId, Constants.SEARCH_DATE_MONTH));
    ...
    public User registerByThird(RegisterThirdUserRequest thirdUserRequest, String type) {
        User user = new User();
        user.setAccount(DigestUtils.md5Hex(MallUtil.getUuid() + DateUtil.getNowTime()));
        ...
        user.setAddres(thirdUserRequest.getCountry() + "," + thirdUserRequest.getProvince() + ","
            + thirdUserRequest.getCity());
        save(user);
        return user;
    }
    ...
    public boolean spread(Integer currentUserId, Integer spreadUserId) {
        ...
        if (null == currentUser) {
            throw new MallException("用户id:" + currentUserId + "不存在");
        }
        User spreadUser = userDao.selectById(spreadUserId);
        if (null == spreadUser) {
            throw new MallException("用户id:" + spreadUserId + "不存在");
        }
        // 检查是否是推广员
        if (!spreadUser.getIsPromoter()) {
            throw new MallException("用户id:" + spreadUserId + "不是推广员身份");
        }
        // 检查是否已经有推广关系
        ...
        if (spreadUsers.size() > 0) {
            throw new MallException("推广关系已经存在");
        }
    ...
    private PageInfo<User> getFirstSpreadUserListPage(RetailShopStairUserRequest request,
        PageParamRequest pageParamRequest) {
        Page<User> userPage = PageHelper
            .startPage(pageParamRequest.getPage(), pageParamRequest.getLimit());
        LambdaQueryWrapper<User> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.select(User::getUid, User::getAvatar, User::getNickname, User::getIsPromoter,
            User::getSpreadCount, User::getPayCount);
        queryWrapper.eq(User::getSpreadUid, request.getUid());
        if (StrUtil.isNotBlank(request.getNickName())) {
            queryWrapper.and(e -> e.like(User::getNickname, request.getNickName()).or()
...
//        }
    ...
    private PageInfo<User> getSecondSpreadUserListPage(RetailShopStairUserRequest request,
        ...
        List<Integer> userIds = firstUserList.stream().map(User::getUid).distinct()
...
//        }
     ...
     */
    @Override
    public PageInfo<StoreOrder> getOrderListBySpreadLevel(RetailShopStairUserRequest request,
        PageParamRequest pageParamRequest) {
...
//        }
        ...
        List<Integer> userIds = userList.stream().map(User::getUid).distinct()
            .collect(Collectors.toList());
...
//        lqw.like(User::getPath, request.getUid());
    ...
    public User updateForPromoter(User current) {
        ...
        BigDecimal price = orders.stream().map(e -> e.getPayPrice())
        ...
        if (isPromoter) {
            ...
            if (updateCount <= 0) {
                throw new MallException("升级推广人时 更新用户信息出错");
            }
            return current;
        }
        return current;
    ...
    public boolean checkIsPromoter(BigDecimal price) {
        String brokerageStatus = systemConfigService
            .getValueByKey(Constants.CONFIG_KEY_STORE_BROKERAGE_IS_OPEN);
        if (brokerageStatus.equals("1")) {
            return false;
        } else {
            BigDecimal brokeragePrice = BigDecimal.valueOf(
                Integer.parseInt(systemConfigService.getValueByKey("store_brokerage_price")));
    ...
    public List<User> getTopSpreadPeopleListByDate(String type, PageParamRequest pageParamRequest) {
        ...
        List<Integer> spreadIdList = spreadVoList.stream().map(User::getSpreadUid)
            .collect(Collectors.toList());
    ...
    public void bindSpread(User user, Integer spreadUid) {
        ...
        String distribution = systemConfigService
            .getValueByKey(Constants.CONFIG_KEY_DISTRIBUTION_TYPE);

        //人人分销 + 当前已绑定的id和推广人id不一样
        if (StringUtils.isNotBlank(distribution) && distribution.equals("0") && !user.getSpreadUid()
            .equals(spreadUid)) {
            user.setSpreadUid(spreadUid);
            user.setSpreadTime(DateUtil.nowDateTime());
        }
    ...
    public Boolean editSpread(UserUpdateSpreadRequest request) {
        ...
        if (userId.equals(spreadUid)) {
            throw new MallException("上级推广人不能为自己");
        }
        User user = getById(userId);
        if (ObjectUtil.isNull(user)) {
            throw new MallException("用户不存在");
        ...
        if (user.getSpreadUid().equals(spreadUid)) {
            throw new MallException("当前推广人已经是所选人");
        }
        User spreadUser = getById(spreadUid);
        if (ObjectUtil.isNull(spreadUser)) {
            throw new MallException("上级用户不存在");
        }
     ...
     */
    @Override
    public List<User> findDistributionList(String keywords, String dateLimit,
        String storeBrokerageStatus) {
        LambdaQueryWrapper<User> lqw = new LambdaQueryWrapper<>();
        if (storeBrokerageStatus.equals("1")) {
            lqw.eq(User::getIsPromoter, true);
        }
    ...
    public Boolean updateUser(UserRequest userRequest) {
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            updateById(user);
            if (ObjectUtil.isNotNull(userRequest.getLevel()) && !tempUser.getLevel()
                .equals(userRequest.getLevel())) {
                // 修改用户等级表
                ...
                SystemUserLevel systemUserLevel = systemUserLevelService
                    .getByLevelId(userRequest.getLevel());
                ...
                newLevel.setMark(StrUtil.format("尊敬的用户 {},在{}管理员调整会员等级成为{}", tempUser.getNickname(),
                    DateUtil.nowDateTimeStr(), systemUserLevel.getName()));
