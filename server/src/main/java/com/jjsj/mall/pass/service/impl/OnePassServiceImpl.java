package com.jjsj.mall.pass.service.impl;
...
import com.jjsj.constants.OnePassConstants;
...
import com.jjsj.utils.OnePassUtil;
import com.jjsj.utils.RedisUtil;
import com.jjsj.utils.ValidateFormUtil;
import com.jjsj.mall.pass.request.MealCodeRequest;
import com.jjsj.mall.pass.request.OnePassLoginRequest;
import com.jjsj.mall.pass.request.OnePassRegisterRequest;
import com.jjsj.mall.pass.request.OnePassUpdateRequest;
import com.jjsj.mall.pass.request.OnePassUserRecordRequest;
import com.jjsj.mall.pass.request.ServiceOpenRequest;
import com.jjsj.mall.pass.service.OnePassService;
import com.jjsj.mall.pass.vo.OnePassLoginVo;
import com.jjsj.mall.pass.vo.OnePassLogisticsQueryVo;
...
public class OnePassServiceImpl implements OnePassService {
    ...
    public Object sendUserCode(String phone, Integer types) {
        ...
        return onePassUtil
            .postFrom(OnePassConstants.ONE_PASS_API_URL + OnePassConstants.REGISTER_CAPTCHA_URI,
                map, null);
    }
    ...
    public String register(OnePassRegisterRequest registerRequest) {
        ...
        JSONObject post = onePassUtil
            .postFrom(OnePassConstants.ONE_PASS_API_URL + OnePassConstants.USER_REGISTER_URI, map,
                null);

        // 获取token存入Redis
        String accessToken = OnePassConstants.ONE_PASS_USER_TOKEN_PREFIX
            .concat(post.getJSONObject("data").getString("access_token"));
        Long expiresIn = post.getJSONObject("data").getLong("expires_in");
        expiresIn = expiresIn - DateUtil.getTime();
        String secret = SecureUtil
            .md5(registerRequest.getPhone() + SecureUtil.md5(registerRequest.getPassword()));
        redisUtil
    ...
    public MyRecord login(OnePassLoginRequest request) {
        ...
        String secret = SecureUtil
            .md5(request.getAccount() + SecureUtil.md5(request.getPassword()));
        param.add("secret", secret);
        JSONObject jsonObject = onePassUtil
            .postFrom(OnePassConstants.ONE_PASS_API_URL + OnePassConstants.USER_LOGIN_URI, param,
                null);
        setConfigSmsInfo(request.getAccount(), request.getPassword());

        String accessToken = OnePassConstants.ONE_PASS_USER_TOKEN_PREFIX
            .concat(jsonObject.getJSONObject("data").getString("access_token"));
    ...
    private JSONObject getInfo() {
        ...
        JSONObject jsonObject = onePassUtil
            .postFrom(OnePassConstants.ONE_PASS_API_URL + OnePassConstants.USER_INFO_URI, null,
                header);
    ...
    public Boolean logOut() {
        ...
        boolean exists = redisUtil.exists(
            StrUtil.format(OnePassConstants.ONE_PASS_TOKEN_KEY_PREFIX, loginVo.getSecret()));
        if (exists) {
            redisUtil.remove(
                StrUtil.format(OnePassConstants.ONE_PASS_TOKEN_KEY_PREFIX, loginVo.getSecret()));
        }
    ...
    public Boolean updatePassword(OnePassUpdateRequest request) {
        ...
        onePassUtil
            .postFrom(OnePassConstants.ONE_PASS_API_URL + OnePassConstants.USER_UPDATE_PASSWORD_URI,
                map, null);
    ...
    public Boolean updatePhone(OnePassUpdateRequest request) {
        ...
        onePassUtil
            .postFrom(OnePassConstants.ONE_PASS_API_URL + OnePassConstants.USER_UPDATE_PHONE_URI,
                map, header);
    ...
    public JSONObject mealList(String type) {
        if (!validateMealType(type)) {
            throw new MallException("请选择正确的套餐类型");
        }
        ...
        JSONObject jsonObject = onePassUtil
            .postFrom(OnePassConstants.ONE_PASS_API_URL + OnePassConstants.ONE_PASS_MEAL_LIST_URI,
                params, header);
    ...
    public JSONObject mealCode(MealCodeRequest request) {
        if (!validateMealType(request.getType())) {
            throw new MallException("请选择正确的套餐类型");
        }
        // 获取用户详情
        String token = onePassUtil.getToken();
        ...
        if (!isGrant) {
            throw new MallException("请先开通对应的服务");
        }
        ...
        JSONObject post = onePassUtil
            .postFrom(OnePassConstants.ONE_PASS_API_URL + OnePassConstants.ONE_PASS_MEAL_CODE_URI,
                params, header);
    ...
    public JSONObject userRecord(OnePassUserRecordRequest request) {
        if (!validateMealType(request.getType())) {
            throw new MallException("请选择正确的记录类型");
        }
        MultiValueMap<String, Object> params = new LinkedMultiValueMap<>();
        params.add("type", request.getType());
...
//            int status = 3;// 查询全部状态
        ...
        HashMap<String, String> header = onePassUtil.getCommonHeader(token);
        JSONObject jsonObject = onePassUtil
            .postFrom(OnePassConstants.ONE_PASS_API_URL + OnePassConstants.ONE_PASS_USER_RECORD_URI,
                params, header);
        return jsonObject.getJSONObject("data");
    }
    ...
    public JSONObject copyGoods(String url) {
        ...
        MultiValueMap<String, Object> params = new LinkedMultiValueMap<>();
        params.add("url", url);
        JSONObject jsonObject = onePassUtil.postFrom(
    ...
    public MyRecord expressDump(MyRecord record) {
        ...
        JSONObject post = onePassUtil.postFrom(
            OnePassConstants.ONE_PASS_API_URL + OnePassConstants.ONE_PASS_API_EXPRESS_DUMP_URI,
            params, header);
    ...
    public OnePassLogisticsQueryVo exprQuery(String expressNo, String com) {
        ...
        JSONObject post = onePassUtil.postFrom(
            OnePassConstants.ONE_PASS_API_URL + OnePassConstants.ONE_PASS_API_EXPRESS_QUEARY_URI,
            params, header);
    ...
    public Boolean beforeUpdatePhoneValidator(OnePassLoginRequest request) {
        ...
        if (!loginVo.getAccount().equals(request.getAccount())) {
            throw new MallException("账号不匹配");
        }
        String secret = SecureUtil
            .md5(request.getAccount() + SecureUtil.md5(request.getPassword()));
        if (!loginVo.getSecret().equals(secret)) {
            throw new MallException("密码不匹配");
        }
        return Boolean.TRUE;
    }
    ...
    private void expressOpenValidate(ServiceOpenRequest request) {
        if (StrUtil.isBlank(request.getCom())) {
            throw new MallException("请选择快递公司");
        }
        if (StrUtil.isBlank(request.getTempId())) {
            throw new MallException("请选择快递模板");
        }
        if (StrUtil.isBlank(request.getToName())) {
            throw new MallException("请填写寄件人姓名");
        }
        if (StrUtil.isBlank(request.getToTel())) {
            throw new MallException("请输入寄件人手机号码");
        }
        if (StrUtil.isBlank(request.getToAddress())) {
            throw new MallException("请填写寄件人详细地址");
        }
        if (StrUtil.isBlank(request.getSiid())) {
            throw new MallException("请填写云打印机编号");
        }
    ...
    private Boolean copyOpen() {
        ...
        HashMap<String, String> header = onePassUtil.getCommonHeader(token);
        onePassUtil
            .postFrom(OnePassConstants.ONE_PASS_API_URL + OnePassConstants.ONE_PASS_COPY_OPEN_URI,
                null, header);
        return Boolean.TRUE;
    }
    ...
    private Boolean expressOpen(ServiceOpenRequest request) {
        ...
        if (request.getType().equals(OnePassConstants.ONE_PASS_MEAL_TYPE_EXPR)) {// 物流
            onePassUtil.postFrom(
                OnePassConstants.ONE_PASS_API_URL + OnePassConstants.ONE_PASS_EXPRESS_OPEN_URI,
                null, header);
            return Boolean.TRUE;
        }
        ...
        if (ObjectUtil.isNull(express)) {
            throw new MallException("没有找到对应的快递公司");
        }
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            // 快递公司id
            systemConfigService
                .updateOrSaveValueByName("config_export_id", express.getId().toString());
            // 快递公司模板id
            systemConfigService
                .updateOrSaveValueByName("config_export_temp_id", request.getTempId());
            // 快递公司编码
            systemConfigService.updateOrSaveValueByName("config_export_com", request.getCom());
            // 发货人姓名
            systemConfigService
                .updateOrSaveValueByName("config_export_to_name", request.getToName());
            // 发货人电话
            systemConfigService.updateOrSaveValueByName("config_export_to_tel", request.getToTel());
            // 发货人详细地址
            systemConfigService
                .updateOrSaveValueByName("config_export_to_address", request.getToAddress());
            // 电子面单打印机编号
            systemConfigService.updateOrSaveValueByName("config_export_siid", request.getSiid());
            return Boolean.TRUE;
        });
        if (Boolean.FALSE.equals(execute)) {
            throw new MallException("保存平台电子面单打印信息失败");
        }
        ...
        onePassUtil.postFrom(
            OnePassConstants.ONE_PASS_API_URL + OnePassConstants.ONE_PASS_EXPRESS_OPEN_URI, null,
            header);
        return Boolean.TRUE;
    }
    ...
    private Boolean smsOpen(ServiceOpenRequest request) {
        ...
        onePassUtil
            .postFrom(OnePassConstants.ONE_PASS_API_URL + OnePassConstants.ONE_PASS_SMS_OPEN_URI,
                params, header);
        return Boolean.TRUE;
    }
    ...
    private void setConfigSmsInfo(String account, String password) {
        ...
        if (!accountResult || !tokenResult) {
            throw new MallException("数据更新失败！");
        }
    }
}
