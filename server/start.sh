...
APP_PATH=/Users/kevin/Code/Project/mall/server/target

#jar包文件名称
APP_NAME=$APP_PATH/Mall.jar

#日志文件名称
LOG_FILE=$APP_PATH/mall_out.log
...
# 启动jar包，指向日志文件，2>&1 & 表示打开或指向同一个日志文件  --spring.profiles.active=prod 启动 prod环境
  ...
  nohup java -jar $APP_NAME $APP_YML  > mall_out.log 2>&1 &
  echo "正在发布中，请稍后......"
  sleep 10s
  ...
  while [ -f $LOG_FILE ]
  do
      success=`grep "Started MallApplication in " $LOG_FILE`
      if [[ "$success" != "" ]]
      then
#          echo "Mall start ........."
          break
